...
async function fetchOfferDetails(fileId: string): Promise<OfferDetails> {
  ...
  const response = await fetch(`https://external.iconluxury.today/api/luxurymarket/supplier/offers/${fileId}`, {
    method: "GET",
    headers: {
...
async function submitOfferForReviewApi(fileId: string): Promise<{ message: string }> { // Expecting a message on success
  ...
  const response = await fetch(`https://external.iconluxury.today/submitOffer`, {
    method: "POST",
